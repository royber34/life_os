...
- You're looking for a one-size-fits-all template to copy. The methodology generalizes; the templates in `templates/` are starting points for the structures it produces, not finished files. The final file shape depends on your work.
...
For each major project with substantial infrastructure context, create or update a project-level CLAUDE.md. These auto-load only when you `cd` into the project, so they don't bloat unrelated sessions.
...
In one case mid-restructure, I had told the AI *"I disagree with merging these two bullets, slightly"*, and the AI merged them anyway in the next draft, because the surface logic was tempting. The drift check caught it.
...
.
├── README.md                        ← This file
├── install.sh                       ← One-line bash installer for the skill
├── install.ps1                      ← One-line PowerShell installer for the skill
├── skill/
